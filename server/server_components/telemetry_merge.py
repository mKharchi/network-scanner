...
_SEED_TOP_LEVEL_FIELDS = {"client_id", "sync_timestamp", "updated_devices"}
_SEED_DEVICE_FIELDS = _DEVICE_FIELDS - {"activity"}
_FORBIDDEN_FIELDS = {
...
def validate_seed_payload(payload: Any) -> Dict[str, Any]:
    """Validate a registration-time device-only v2 inventory seed."""
    if not isinstance(payload, dict):
        raise ValueError("Telemetry seed payload must be an object")
    _ensure_allowed(set(payload), _SEED_TOP_LEVEL_FIELDS, "payload")
    client_id = payload.get("client_id")
    devices = payload.get("updated_devices")
    if not isinstance(client_id, str) or not client_id.strip():
        raise ValueError("client_id is required")
    ...
    for index, source in enumerate(devices):
        if not isinstance(source, dict):
            raise ValueError(f"updated_devices[{index}] must be an object")
        _ensure_allowed(set(source), _SEED_DEVICE_FIELDS, f"updated_devices[{index}]")
        discovery = source.get("discovery", {})
        if not isinstance(discovery, dict):
            raise ValueError(f"updated_devices[{index}].discovery must be an object")
        normalized_devices.append({
...
def _upsert_device(cursor, *, client_id: str, device: Dict[str, Any], default_last_seen: datetime) -> None:
    last_seen = device["last_seen"] or default_last_seen
    cursor.execute(
        """
    ...
    cursor.execute(
        ...
        ON DUPLICATE KEY UPDATE
            ...
            updated_at = CURRENT_TIMESTAMP
        """,
        (client_id, device["mac"], device["os_guess"], json.dumps(device["discovery"]), last_seen),
    )
    ...
    try:
        for device in data["updated_devices"]:
            _upsert_device(
                ...
                device=device,
                default_last_seen=data["sync_timestamp"],
            )
...
def merge_telemetry_seed(payload: Any, *, conn=None) -> Dict[str, Any]:
    """Merge a device-only registration seed without creating activity rows."""
    data = validate_seed_payload(payload)
    owns_connection = conn is None
    connection = conn or get_connection()
    if connection is None:
        raise RuntimeError("Database connection unavailable")
    ...
    try:
        ...
        return {
            "status": "ack",
            "client_id": data["client_id"],
            "seeded_devices": len(data["updated_devices"]),
        }
    except Exception:
        connection.rollback()
        raise
