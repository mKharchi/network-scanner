...
import { api, ApiError, type Alert } from "../api/client";
...
export function AlertsPage() {
  ...
  const affectedClients = new Set(
    alerts.map((alert) => alert.client?.id).filter(Boolean),
  ).size;

  const updateAlertStatus = async (status: "ACKNOWLEDGED" | "RESOLVED") => {
    if (!selectedAlertId) return;
    setStatusAction(status);
    setStatusError(null);
    try {
      await api.updateAlertStatus(selectedAlertId, status);
    ...
    <div>
      <div className="page-header operations-page-header">
        <div className="client-page-header__copy">
          ...
          <p className="page-description">
            Investigate operational anomalies, client incidents, and security
            signals across the infrastructure.
          </p>
        </div>
      </div>
      ...
      <div className="operations-metric-grid" aria-label="Alert overview">
        <MetricCard
          ...
          value={alerts.length}
          context="Matching the current filters"
        />
        <MetricCard
          label="New"
          value={newCount}
          valueVariant={newCount > 0 ? "danger" : "success"}
          context="Awaiting operator review"
        />
        <MetricCard
          label="High priority"
          value={highCount}
          valueVariant={highCount > 0 ? "warning" : "success"}
          context="High or critical severity"
        />
        <MetricCard
          label="Affected clients"
          value={affectedClients}
          valueVariant="info"
          context="Distinct client targets"
        />
      </div>
      ...
      <div className="operations-toolbar">
        <div className="operations-toolbar__label">
          ...
          <strong>
            {newCount > 0
              ? `${newCount} event${newCount === 1 ? "" : "s"} need review`
              : "No new events need review"}
          </strong>
        </div>
        <div
        ...
        >
          {alertDetailState.status === "loading" ? (
            ...
            >
              ...
              >
                <SeverityBadge
                  severity={alertDetailState.data.alert.severity}
                />
                ...
                >
                  Detected at{" "}
                  {formatDateTime(alertDetailState.data.alert.detected_at)}
                </span>
              </div>
              ...
              {(alertDetailState.data.alert.status === "NEW" ||
                alertDetailState.data.alert.status === "ACKNOWLEDGED") && (
                <div
                  style={{
                    display: "flex",
                    gap: "var(--space-2)",
                ...
                >
                  {alertDetailState.data.alert.status === "NEW" && (
                    <Button
                      variant="secondary"
                      size="sm"
                      loading={statusAction === "ACKNOWLEDGED"}
              ...
              {statusError && (
                <Notice variant="warning" title="Status update failed">
                  {statusError}
                </Notice>
              )}
      ...
      <div style={{ marginBottom: "var(--space-6)" }}>
        {alertListState.status === "idle" ||
        alertListState.status === "loading" ? (
