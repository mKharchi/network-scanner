...
/** Compact, keyboard-accessible segmented selector used by the floor view. */
      ...
      <div className="location-radio__track">
        {options.map((option) => (
          ...
          >
            <input
              type="radio"
              name={`location-${label.toLowerCase().replace(/\s+/g, "-")}`}
...
function formatUnassignedReason(
  reason: string,
...
export function LocationsPage() {
  ...
  const { state: unassignedState, refetch: refetchUnassigned } = useFetch<{
    items: UnassignedClientQueueItem[];
    total: number;
  }>(
  ...
  const handleReleaseQuarantine = async () => {
    if (!selectedLocation?.client_id || client?.connection.state !== "ISOLATED")
      return;
    if (
      !window.confirm(
  ...
  const assignClientToSeat = async (location: ClientLocation) => {
    ...
    const label =
      ...
      (selectedLocation?.client_id === assigningClientId
        ...
        : null) ||
      assigningClientId;
    if (!window.confirm(`Assign ${label} to ${location.label}?`)) {
      return;
    }
  ...
  const pendingActionLabel =
    pendingBulkAction?.actionType === "SHUTDOWN" ? "shut down" : "restart";
  ...
  return (
    <div>
      <ConfirmDialog
        ...
        confirmLabel={
          pendingActionLabel === "shut down" ? "Shut down" : "Restart"
        }
                          ...
                          <>
                            ...
                            <div className="floor-vis__columns">
                              {tableColumns(table).map((column, index) => (
                                <div
                                  key={String(
                                    column.column ?? column.row ?? index,
                                  )}
                                ...
                                >
                                  ...
                                  <div className="floor-vis__stations">
                                    {column.stations.map((station) => {
                                      ...
                                      const assignment =
                                        resolveLocationAssignment(station);
                                      const method =
                                        assignmentMethodOf(assignment);
              ...
              <div>
                ...
                {assigningClientId && !selectedLocation.client_id && (
                  <div style={{ marginTop: "var(--space-3)" }}>
                    ...
                    >
                      Assign {assigningClient?.hostname || assigningClientId}{" "}
                      here
                    </Button>
                  </div>
                )}
                ...
                >
                  ...
                  <PanelRow
                    label="Column"
                    value={
                      selectedLocation.column ?? selectedLocation.row ?? "—"
                    }
                  />
                  <PanelRow
                    label="Position"
